...
const BlogTitles = () => {
  ...
  const onSubmitHandler = async (e) => {
      ...
      try {
        setLoading(true)
        const prompt = `Generate catchy blog titles for the topic
        ...
        toast.error("An error occurred while generating blog titles");
      }
      setLoading(false);
    };
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        >
          {loading ? (
            <span 
            className="w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin"></span>
          ) : (
            <Hash className="w-5" />
          )}
          Generate Titles
